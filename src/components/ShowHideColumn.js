...
const ShowHideColumn = ({toggle}) => {
  ...
  const [userDetails, setUserDetails] = useState({ userName: null, fullName: null, password: null });

  const handleClose = () => {
    setAnchorEl(null);
    setUserDetails({ userName: null, fullName: null, password: null });
  };
  ...
  return (
    <>
      ...
      <Modal open={open} onClose={handleClose}>
        <Box style={{ ...style }} sx={{ width: 400, backgroundColor: 'whitesmoke', height: 400 }}>
          <Box sx={{ padding: '40px', display: 'flex', flexDirection: 'column' }}>
            ...
            <Button
              variant="outlined"
              disabled={!userDetails.fullName || !userDetails.userName || !userDetails.password}
              onClick={() => {
                createNewUser();
                handleClose();
              }}
